...
use tokio_rustls_fork_shadow_tls::TlsAcceptor;
...
async fn main() {
    ...
    let listener = TcpListener::bind("127.0.0.1:50443").await.expect("unable to listen 127.0.0.1:50443");
...
async fn process_raw_stream(stream: TcpStream, tls_acceptor: TlsAcceptor) -> io::Result<()> {
    ...
    loop {
        ...
        let res = tls_stream.read(buf).await;
        let len: usize = res?;
        if len == 0 {
            // eof
            break;
        ...
        // write all
        let _ = tls_stream.write_all(&buf[n..len]).await;
        n += len;
    }
